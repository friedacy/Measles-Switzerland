...
function_5_year <- function(Kt) {
  ...
  data_cases <- data_Total %>%
    ...
    group_by(Year_gr) %>%
    summarise(Inc_gr_sum = sum(cases_year_r, na.rm =TRUE),
  ...
  data_death <- data_Total %>%
    ...
    mutate(n_year = 1,
           ...
           Year_gr = ifelse(Year_gr>=1877 & Year_gr < 1880, "1877-1879", Year_gr)) %>%
    group_by(Year_gr) %>%
    summarise(Death_gr_sum = sum(death_r, na.rm =TRUE),
              Pop_gr_sum = sum(pop_r,na.rm =TRUE)) %>%
    ungroup()
  # filter( Year_gr == "1935-1939") %>%
...
function_5_year("ZH") %>%
  ...
  save_kable("output/Table2_7.html")

#library(webshot2)
#webshot("output/Table2_7.html", "output/Table1_7.pdf")

Table1 <- function_5_year("ZH")
write.xlsx(Table1,file="output/Table1_7.xlsx",rowNames=FALSE, overwrite = TRUE)
